...
### Fixed
- `validateWebhook()` no longer silently coerces missing `public_invoice_id` to `""` — now throws `SDKValidationError` when critical webhook fields (`invoiceId`, `publicInvoiceId`, `merchantId`) are absent from the payload
- Webhook payloads missing required ID fields were previously accepted by the permissive `types.string()` parser, causing downstream checkout lookup failures with no visible error

### Added
- Webhook validation tests for missing `public_invoice_id` and `invoice_id` fields
...
[0.4.1]: https://github.com/gwop-io/gwop-node/compare/v0.4.0...v0.4.1
[0.4.0]: https://github.com/gwop-io/gwop-node/compare/v0.3.0...v0.4.0
